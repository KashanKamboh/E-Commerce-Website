...
import { ChadarProduct } from "@/app/components/ChadarProducts";
import Footer from "@/app/components/Footer";
import Header from "@/app/components/Header";
import Image from "next/image";


interface ChadarPageProps {
  params: Promise<{ slug: string }>; // Updated to reflect that params is a Promise
}

// Generate static params for all articles
export async function generateStaticParams() {
  return ChadarProduct.map((chadar) => ({
    slug: chadar.slug,
  }));
}

// Blog Page component
export default async function ChadarPage({ params }: ChadarPageProps) {
  const { slug } = await params; // Await the resolution of params

  // Find the article based on the slug
  const chadar = ChadarProduct.find((a) => a.slug === slug);
  ...
  return (
    ...
    <div className=" font-[sans-serif] p-4">
      ...
      <div>
        ...
        <h1 className="text-lg sm:text-2xl font-bold text-gray-800">{chadar.title}</h1>
        <p className="text-gray-800 text-2xl sm:text-3xl font-semibold mt-2"> Rs.{chadar.price}</p>
        <div className="flex items-center gap-4 mt-2">
            ...
            <p className="text-gray-800 text-sm mt-2">{chadar.paragraph}</p>
            <p className="text-gray-700 text-sm mt-2">{chadar.order}</p>
      ...
      <div>
       ...
       <p className="text-lg mb-4 mt-4 text-gray-800">{chadar.content}</p>
      </div>
      
      <div>
        
        <p className="text-lg font-semibold text-gray-700">{chadar.p1}</p>
        <p className="text-lg font-semibold  text-gray-700 mt-2">{chadar.p2}</p>
        <p className="text-lg font-semibold   text-gray-700 mt-2">{chadar.p3}</p>
      </div>
      </div>
      </div>
      </div>
